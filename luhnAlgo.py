...
cardnumVal = cardNum.copy() #copying the card number to another list to manipulate it without changing the original list

for z in range(15):#loop to to double the numbers
    if z % 2 == 0:#checking if the index is even , since we only need every other number starting from index 2 to be doubled
        cardnumVal[z]= cardnumVal[z]*2 #number at that index is doubled
for nums in cardnumVal: #loop to checking if any number is greater than 9
    if nums > 9: #check if number is greater than 9
        ...
        cardnumVal[cardnumVal.index(nums + 9)] = nums #replacing the number in the list with the new value after subtracting 9

for nums in range(1,15): #loop to add all the numbers together , starting from index 1 since index 0 is user input
    sum = sum + cardnumVal[nums] #total
...
print("validation number:", val_number) #printing the validation number to check if everything is working correctly
